...
def get_reward(drone, customer, emergency, depot, delivered, truck, speed, near_customer, near_depot, near_emergency, battery):
    ...
    reward -= 0.5*(distance(drone, truck)/100 - 0.1*(abs(vx) + abs(vy)))
...
for episode in range(episodes):
    ...
    emergency = (700, 150)
    # epsilon = 0.2
    epsilon = max(0.2, 1 - episode / 50000) 
    delivered = False
    near_customer = False
    near_depot = False
    ...
    while not is_within_radius(drone, depot) and not is_within_radius(drone, emergency) and battery > 0:
        ...
        if episode % viz_step == 0 and episode != 0:
            ...
            if action == "move_away" and is_near_congestion(drone):
                move_away_text = font.render(f'Move Away in {dir}', True, BLACK)
                window.blit(move_away_text, (10, 200))
...
plot_x = range(len(total_rewards))
plt.figure(figsize=(10, 6))
plt.plot(plot_x, plot_reward, label="Total Rewards")

# Set labels and title
plt.xlabel('Episode Number')
plt.ylabel('Total Reward')
plt.title('Total Rewards vs. Episode Number')
