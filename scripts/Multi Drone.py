...
for tzr in trust_zone_radii:
    for rr in rewards_ratio:
        ...
        def get_reward(drone1, drone2, customer1, customer2, emergency, depot, delivered1, delivered2, truck, speed1, speed2, near_customer1, near_customer2, near_depot, near_emergency, battery1, battery2):
        reward = 0
    
        for obstacle in obstacles:
            if obstacle.collidepoint(drone1) or obstacle.collidepoint(drone2):
                return -100
        
        if battery1 <= 0 or battery2 <= 0:
            return -10000
        ...
        if is_within_radius(drone1, depot) or is_within_radius(drone1, emergency):
            reward += 50
        if is_within_radius(drone2, depot) or is_within_radius(drone2, emergency):
            reward += 50
    
        # Penalty for being in congestion zones
        if is_in_congestion_zone(drone1) or is_in_congestion_zone(drone2):
            reward -= 50
